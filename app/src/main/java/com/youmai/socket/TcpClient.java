...
public class TcpClient extends PduUtil implements Runnable {
     ...
     * 协议监听
     */
    private final ConcurrentHashMap<Short, ReceiveListener> mNotifyListener;
    ...
    public TcpClient(Context context) {
        ...
        mNotifyListener = new ConcurrentHashMap<>();
     ...
     * 发送协议接口
     */
    public synchronized void sendProto(PduBase pdu) {
        sendPdu(pdu);
    }
     ...
     * 发送协议接口
     */
    public synchronized void sendProto(PduBase pdu, short rspId, ReceiveListener callback) {
    ...
    public void setNotifyListener(short commandId, ReceiveListener callback) {
        if (callback != null) {
            mNotifyListener.put(commandId, callback);
        }
    }

    public void clearNotifyListener(short commandId) {
        mNotifyListener.remove(commandId);
    }
    ...
    public void OnRec(final PduBase pduBase) {
        ...
        final ByteBuffer buffer = ByteBuffer.wrap(pduBase.params);
        buffer.order(ByteOrder.LITTLE_ENDIAN);

        buffer.put(pduBase.params);
            ...
            public void run() {
                ...
                if (callback != null) {
                    if (buffer.remaining() > 0) {
                        callback.OnRec(buffer);
                        mHandler.removeCallbacks(callback.getRunnable());
                        mCommonListener.remove(key);
                    } else {
                        Log.e(TAG, "tcp rec common buffer n params is empty");
                    }

                } else {
                    if (buffer.remaining() > 0) {
                        OnCallback(key, buffer);
                    } else {
                        Log.e(TAG, "tcp rec notify buffer n params is empty");
                    }
                }
    ...
    @Override
    public void OnCallback(short key, ByteBuffer buffer) {
        ReceiveListener callback = mNotifyListener.get(key);
        if (callback != null) {
            callback.OnRec(buffer);
        }
    }
    ...
    private void heatBeat() {
        ...
        HuxinSdkManager.instance().chargerHeart(new ReceiveListener() {
            @Override
