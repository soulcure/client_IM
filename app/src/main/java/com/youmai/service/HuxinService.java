...
public class HuxinService extends Service {
    ...
    public class HuxinServiceBinder extends Binder {
        ...
        public void sendProto(short commandId, ByteBuffer buffer) {
            buffer.flip();
            int limit = buffer.remaining();
            byte[] params = null;
            if (limit > 0) {
                params = new byte[limit];
                buffer.get(params);
            }

            PduBase pdu = ProtoCommandId.buildProto(commandId, params);
            mClient.sendProto(pdu);
        }



        public void sendProto(short commandId, ByteBuffer buffer, short rspId, ReceiveListener callback) {
            buffer.flip();
            int limit = buffer.remaining();
            byte[] params = null;
            if (limit > 0) {
            ...
            PduBase pdu = ProtoCommandId.buildProto(commandId, params);
            mClient.sendProto(pdu, rspId, callback);
        }

        public void startHeartBeat() {
            mClient.startHeartBeat();
        }


        public void setNotifyListener(short commandId, ReceiveListener callback) {

            mClient.setNotifyListener(commandId, callback);
        }

        public void clearNotifyListener(short commandId) {
            mClient.clearNotifyListener(commandId);
        }
